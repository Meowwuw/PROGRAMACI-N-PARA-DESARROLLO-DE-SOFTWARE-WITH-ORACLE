...
-- (Opcional) Descomentar para empezar de cero si ya ejecuto scripts antes:
...
-- Extension para cifrar contrasenas (bcrypt) desde SQL
...
CREATE TABLE usuario (
    ...
    id_rol            INT NOT NULL DEFAULT 4 REFERENCES rol(id_rol), -- 4 = apoderado
    id_apoderado      INT UNIQUE REFERENCES apoderado(id_apoderado),
    id_veterinario    INT UNIQUE REFERENCES veterinario(id_veterinario),
...
CREATE INDEX idx_registro_login_usuario ON registro_login(id_usuario);
CREATE INDEX idx_registro_login_fecha   ON registro_login(fecha_hora);
...
INSERT INTO usuario (username, email, password_hash, id_rol, id_apoderado, id_veterinario) VALUES
...
INSERT INTO registro_login (id_usuario, username_intentado, exito, ip_origen, dispositivo) VALUES
...
SELECT COUNT(*) AS total_registro_login FROM registro_login;
...
LEFT JOIN apoderado a   ON u.id_apoderado   = a.id_apoderado
...
-- UPDATE usuario SET ultimo_login = NOW(), intentos_fallidos = 0
-- WHERE id_usuario = $1;
-- INSERT INTO registro_login (id_usuario, username_intentado, exito, ip_origen, dispositivo)
-- VALUES ($1, $2, TRUE, $3, $4);
...
-- WHERE username = $1;
-- INSERT INTO registro_login (id_usuario, username_intentado, exito, ip_origen, dispositivo)
-- VALUES ((SELECT id_usuario FROM usuario WHERE username = $1), $1, FALSE, $2, $3);
...
GRANT SELECT ON consulta, mascota, tratamiento TO db_veterinario;

-- 6) APODERADO: veterinario, tratamiento, consulta y mascota
GRANT SELECT ON veterinario, tratamiento, consulta, mascota TO db_apoderado;
...
-- SELECT * FROM mascota;        -- OK
-- SELECT * FROM usuario;        -- ERROR: permission denied
-- SELECT * FROM veterinario;    -- ERROR: permission denied
-- RESET ROLE;
--
-- SET ROLE db_apoderado;
-- SELECT * FROM veterinario;    -- OK
-- SELECT * FROM rol;            -- ERROR: permission denied
-- RESET ROLE;
--
-- SET ROLE db_admin;
