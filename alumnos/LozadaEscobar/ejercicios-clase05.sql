...
--      RESPUESTA:
   SELECT * FROM cliente WHERE correo = NULL;
-- En SQL, NULL representa la ausencia de un valor (un estado desconocido), no un valor en sí mismo.
-- Por lo tanto, no se puede comparar usando el operador de igualdad (=), ya que evaluar "NULL = NULL" 
-- resulta en UNKNOWN (desconocido) en lugar de TRUE. Para consultar valores nulos se debe usar la sintaxis "IS NULL".
