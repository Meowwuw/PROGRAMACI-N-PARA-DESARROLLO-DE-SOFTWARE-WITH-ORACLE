...
SELECT 
    UPPER(SUBSTRING(nombre FROM 1 FOR 3)) || '-' || UPPER(SUBSTRING(apellido FROM 1 FOR 1)) AS codigo_cliente
FROM cliente;
...
SELECT 
    id_cliente,
    INITCAP(nombre) || ' ' || INITCAP(apellido) AS nombre_completo,
...
('Ana', 'Gómez', 'ana.gomez@email.com', '912345678'),
('Luis', 'Torres', 'luis.torres@email.com', '955112233'),
('Sofía', 'Vargas', 'sofia.vargas@email.com', '944332211'),
('Diego', 'Ramírez', 'diego.ramirez@email.com', '966778899');
...
(4, 'Desarrollador Backend', 3500.00, '2025-01-15'),
(5, 'Analista de Sistemas', 4000.00, '2024-06-01');
...
INSERT INTO empleado (id_persona, cargo, sueldo, fecha_ingreso) VALUES
(1, 'Soporte Técnico', 2500.00, '2026-02-01');
...
--
-- RESPUESTA:
--           Pudo aparecer en ambas tablas sin repetir datos personales gracias al uso de 
--			 la Clave Primaria (PK) de PERSONA actuando simultáneamente como Clave Foránea (FK) 
--			 en las tablas de subtipo. Las tablas CLIENTE_SUBTIPO y EMPLEADO solo almacenan 
--			 la referencia (id_persona) y sus atributos específicos. La información general 
--			(nombre, apellido, correo, teléfono) se mantiene centralizada en la tabla PERSONA, 
--			evitando la redundancia mediante consultas con JOIN.
...
--
-- RESPUESTA:
--			 Es una jerarquía SOLAPADA (overlapping). Lo comprobamos en el Paso 4 al insertar 
--			 el id_persona = 1 en la tabla EMPLEADO sin eliminarlo de CLIENTE_SUBTIPO. 
--			 Debido a que las tablas de subtipo no tienen una restricción que impida la 
--			 existencia simultánea de un mismo ID en ambas, la base de datos permite que una 
--			 entidad adopte los dos roles al mismo tiempo.
