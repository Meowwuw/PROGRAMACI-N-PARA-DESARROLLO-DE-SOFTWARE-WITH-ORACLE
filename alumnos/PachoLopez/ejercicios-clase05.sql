...
select nombre, producto from producto where stock = 0;
...
select * from cliente;
select nombre, apellido, ciudad from cliente where ciudad = 'Huanuco' or ciudad = 'Lima' or ciudad = 'Pucallpa';
...
--     RESPUESTA: (Por que NULL significa un valor que no entra en ninguna categoria)
...
select * from producto;
select nombre from producto where nombre ilike '%Michi%';
...
--      RESPUESTA: (Por que no interpreta NULL como un valor normal, si no como un valor nulo o desconocido)
...
select * from producto;
select nombre, precio, categoria from producto where precio < 150;
...
select id_pedido p as pedido,
    fecha p as fecha_pedido,
    apellido c as apellido_cliente,
    ciudad c as ciudad_cliente
from pedido p
join cliente c on p.ide_cliente = c.id_cliente
...
select 
    ...
    d.cantidad,
    d.precio_unit AS precio_cobrado,
    (d.cantidad * d.precio_unit) AS subtotal
from detalle_pedido d
join producto pr on d.id_producto = pr.id_producto
where d.id_pedido = 20
