-- =====================================================================
-- CLASE 05 · TALLER DE CONSULTAS
-- PASO 0 · BASE DE PRACTICA COMPARTIDA
-- ---------------------------------------------------------------------
-- Ejecuta este archivo COMPLETO con Alt + X, una sola vez, al empezar.
-- Crea TecnoMichiStore con datos suficientes.
-- Todos vamos a tener exactamente los mismos datos: si tu resultado no
-- coincide con el del proyector, la consulta esta mal.
-- =====================================================================

CREATE SCHEMA IF NOT EXISTS michistore;
SET search_path TO michistore;

--DROP TABLE IF EXISTS detalle_pedido;
--DROP TABLE IF EXISTS pedido;
--DROP TABLE IF EXISTS producto;
--DROP TABLE IF EXISTS cliente;

CREATE TABLE cliente (
    id_cliente SERIAL      PRIMARY KEY,
    nombre     VARCHAR(50) NOT NULL,
    apellido   VARCHAR(50) NOT NULL,
    correo     VARCHAR(80),
    telefono   VARCHAR(15),
    ciudad     VARCHAR(40) NOT NULL
);

CREATE TABLE producto (
    id_producto SERIAL        PRIMARY KEY,
    nombre      VARCHAR(80)   NOT NULL,
    categoria   VARCHAR(40),
    precio      NUMERIC(10,2) NOT NULL,
    stock       INT           NOT NULL DEFAULT 0
);

CREATE TABLE pedido (
    id_pedido  SERIAL PRIMARY KEY,
    id_cliente INT    NOT NULL REFERENCES cliente(id_cliente),
    fecha      DATE   NOT NULL,
    estado     VARCHAR(20) NOT NULL
);

CREATE TABLE detalle_pedido (
    id_detalle  SERIAL PRIMARY KEY,
    id_pedido   INT    NOT NULL REFERENCES pedido(id_pedido),
    id_producto INT    NOT NULL REFERENCES producto(id_producto),
    cantidad    INT    NOT NULL,
    precio_unit NUMERIC(10,2) NOT NULL
);

-- ---------------------------------------------------------------------
-- CLIENTES  (10)   Ojo: algunos sin correo o sin telefono, a proposito
-- ---------------------------------------------------------------------
INSERT INTO cliente (nombre, apellido, correo, telefono, ciudad) VALUES
('Ana',     'Quispe',   'ana.quispe@gmail.com',    '961234567', 'Huanuco'),
('Luis',    'Ramirez',  'luis.ramirez@hotmail.com','962345678', 'Lima'),
('Rosa',    'Tello',     NULL,                     '963456789', 'Huanuco'),
('Carlos',  'Gonzales', 'carlos.g@gmail.com',       NULL,       'Pucallpa'),
('Ani',     'Garcia',   'ani.garcia@outlook.com',  '965678901', 'Lima'),
('Edgar',   'Angulo',   'edgar.angulo@gmail.com',  '966789012', 'Pucallpa'),
('Genesis', 'Lozada',    NULL,                      NULL,       'Huanuco'),
('Jhoau',   'Zegarra',  'jhoau.z@gmail.com',       '968901234', 'Lima'),
('Fredi',   'Trejo',    'fredi.trejo@hotmail.com', '969012345', 'Pucallpa'),
('Susan',   'Quispe',   'susan.q@GMAIL.COM',       '970123456', 'Huanuco');

-- ---------------------------------------------------------------------
-- PRODUCTOS  (12)   Uno sin categoria, dos sin stock
-- ---------------------------------------------------------------------
INSERT INTO producto (nombre, categoria, precio, stock) VALUES
('Laptop Ryzen 5',           'Computo',     2499.00,  8),
('Mouse Michi inalambrico',  'Accesorios',    59.90, 40),
('Monitor 24 pulgadas',      'Monitores',    749.50, 12),
('Teclado mecanico Michi',   'Accesorios',   189.00,  0),
('Laptop Core i7',           'Computo',     3200.00,  3),
('Audifonos gamer',          'Accesorios',   129.90, 25),
('Monitor 27 curvo',         'Monitores',   1150.00,  5),
('Impresora multifuncional', 'Oficina',      680.00,  0),
('Disco SSD 1TB',            'Componentes',  320.00, 18),
('Memoria RAM 16GB',         'Componentes',  265.00,  9),
('Webcam HD',                'Accesorios',    95.00, 30),
('Alfombrilla Michi XL',      NULL,           45.00, 22);

-- ---------------------------------------------------------------------
-- PEDIDOS  (20)   De junio a setiembre, tres estados distintos
-- ---------------------------------------------------------------------
INSERT INTO pedido (id_cliente, fecha, estado) VALUES
( 1, '2026-06-03', 'ENTREGADO'),
( 2, '2026-06-11', 'ENTREGADO'),
( 4, '2026-06-19', 'CANCELADO'),
( 1, '2026-06-27', 'ENTREGADO'),
( 5, '2026-07-02', 'ENTREGADO'),
( 3, '2026-07-08', 'PENDIENTE'),
( 6, '2026-07-15', 'ENTREGADO'),
( 2, '2026-07-21', 'CANCELADO'),
( 8, '2026-07-29', 'ENTREGADO'),
( 1, '2026-08-04', 'ENTREGADO'),
( 5, '2026-08-09', 'PENDIENTE'),
(10, '2026-08-14', 'ENTREGADO'),
( 4, '2026-08-18', 'ENTREGADO'),
( 6, '2026-08-23', 'CANCELADO'),
( 8, '2026-08-27', 'PENDIENTE'),
( 3, '2026-08-31', 'ENTREGADO'),
(10, '2026-09-02', 'PENDIENTE'),
( 2, '2026-09-06', 'ENTREGADO'),
( 5, '2026-09-11', 'PENDIENTE'),
( 1, '2026-09-15', 'ENTREGADO');

-- ---------------------------------------------------------------------
-- DETALLE  (34)   precio_unit a veces distinto del precio actual:
--                 son ventas viejas, con la tarifa de ese momento
-- ---------------------------------------------------------------------
INSERT INTO detalle_pedido (id_pedido, id_producto, cantidad, precio_unit) VALUES
( 1,  1, 1, 2350.00), ( 1,  2, 2,   55.00),
( 2,  3, 1,  720.00), ( 2, 11, 1,   95.00),
( 3,  5, 1, 3100.00),
( 4,  9, 2,  310.00), ( 4, 10, 1,  260.00),
( 5,  2, 3,   59.90), ( 5,  6, 1,  129.90),
( 6,  7, 1, 1150.00),
( 7,  1, 1, 2499.00), ( 7,  9, 1,  320.00),
( 8,  4, 2,  189.00),
( 9,  3, 2,  749.50), ( 9,  2, 1,   59.90),
(10,  6, 2,  129.90), (10, 11, 1,   95.00),
(11,  5, 1, 3200.00),
(12,  2, 4,   59.90), (12,  4, 1,  189.00),
(13,  9, 1,  320.00), (13, 10, 2,  265.00),
(14,  7, 1, 1150.00),
(15,  1, 1, 2499.00),
(16,  3, 1,  749.50), (16,  6, 1,  129.90),
(17, 11, 2,   95.00),
(18,  2, 5,   59.90), (18,  9, 1,  320.00),
(19, 10, 1,  265.00),
(20,  1, 1, 2499.00), (20,  3, 1,  749.50),
(20,  2, 2,   59.90), (20,  6, 1,  129.90);

-- ---------------------------------------------------------------------
-- COMPROBACION · si algun numero no coincide, vuelve a ejecutar el archivo
-- ---------------------------------------------------------------------
SELECT 'cliente'        AS tabla, COUNT(*) AS filas FROM cliente
UNION ALL SELECT 'producto',       COUNT(*) FROM producto
UNION ALL SELECT 'pedido',         COUNT(*) FROM pedido
UNION ALL SELECT 'detalle_pedido', COUNT(*) FROM detalle_pedido;

-- Debe dar:  cliente 10 · producto 12 · pedido 20 · detalle_pedido 34


-- =====================================================================
-- CLASE 06 · FUNCIONES DE TEXTO Y JERARQUÍAS
-- Taller práctico · PostgreSQL + DBeaver
-- ---------------------------------------------------------------------
-- PARTE 1: continúa trabajando sobre TecnoMichiStore.
-- Antes de empezar, debes tener ejecutado 05a-base-michistore.sql.
--
-- PARTE 2: crearás una jerarquía nueva PERSONA → CLIENTE / EMPLEADO.
-- No borres el Reto 01 cuando empieces el Reto 02.
-- =====================================================================

SET search_path TO michistore;

SELECT current_schema();


-- =====================================================================
-- GRUPO A · NORMALIZAR Y PRESENTAR TEXTO
-- =====================================================================

-- A1. Nombre y apellido en MAYÚSCULAS
select
    nombre,
    apellido,
    upper(nombre) as nombre_mayuscula,
    upper(apellido) as apellido_mayuscula
from cliente;


-- A2. Correo original y correo en minúsculas
select
    correo as correo_original,
    lower(correo) as correo_minuscula
from cliente
where correo is not null;


-- A3. Nombre y apellido como una sola columna
select
    initcap(nombre || ' ' || apellido) as cliente_formateado
from cliente;


-- A4. Texto sin espacios al inicio y al final
select
    trim('   PostgreSQL en DBeaver   ') as texto_limpio;


-- A5. Nombre del producto y cantidad de caracteres
select 
	nombre,
    length(nombre) AS longitud_nombre
from producto
order  by longitud_nombre desc;


-- A6. Ficha del producto
select
    nombre || ' | ' || categoria as ficha_producto
from producto
where categoria is not null;


-- =====================================================================
-- GRUPO B · EXTRAER, BUSCAR Y REEMPLAZAR
-- =====================================================================

-- B7. Primeros 6 caracteres del nombre
select
    nombre,
    substring(nombre from 1 for 6) as inicio_nombre
from producto;


-- B8. Posición del @ en el correo
select
    nombre,
    correo,
    position('@' in correo) as posicion_arroba
from cliente
where correo is not null;


-- B9. Reemplazar Michi por MichiStore
select
    nombre,
    replace(nombre, 'Michi', 'MichiStore') as nombre_modificado
from producto
where nombre like '%Michi%';


-- B10. Mostrar solamente el dominio del correo
select
    nombre,
    correo,
    substring(
        correo
        from position('@' in correo) + 1
    ) as dominio
from cliente
where correo is not null;


-- B11. Productos de la categoría Accesorios
select
    'PRODUCTO: ' || nombre as etiqueta
from producto
where categoria = 'Accesorios';


-- B12. Clientes cuyo correo sea Gmail
-- No usamos ILIKE
select
    nombre,
    apellido,
    correo
from cliente
where lower(correo) like '%@gmail.com';


-- =====================================================================
-- GRUPO C · COMBINAR FUNCIONES DE TEXTO
-- =====================================================================

-- C13. Código del cliente
-- Primeras 3 letras del nombre + primera letra del apellido
select
    upper(
        substring(nombre from 1 for 3)
        || '-'
        || substring(apellido from 1 for 1)
    ) as codigo_cliente
from cliente;


-- C14. Productos con más de 18 caracteres
select
    nombre,
    length(nombre) as cantidad_caracteres
from producto
where length(nombre) > 18;


-- C15. Nombre completo y ciudad
select
    initcap(nombre || ' ' || apellido)
    || ' - ' ||
    initcap(ciudad) as cliente_ciudad
from cliente;


-- C16. Detectar posibles espacios al inicio o final
select
    correo as correo_original,
    trim(correo) as correo_limpio
from cliente
where correo is not null
  and correo <> trim (correo);


-- RESPUESTA C16:
-- TRIM no modifica el dato guardado en la tabla.
-- Solamente modifica el resultado mostrado por la consulta.
-- Para modificar el dato almacenado se necesitaría utilizar UPDATE.


-- =====================================================================
-- RETO 01 · LIMPIEZA DE DATOS
-- =====================================================================
select
    id_cliente,
   initcap(
        trim(nombre) || ' ' || trim(apellido)
    ) as nombre_completo,
    lower(
        trim(correo)
    ) as correo_limpio,
    upper(
        substring(nombre from 1 for 2)
        || id_cliente::text
    ) as codigo
from cliente
where correo is not null;


-- =====================================================================
-- RETO 02 · IMPLEMENTAR UNA JERARQUÍA
-- PERSONA → CLIENTE / EMPLEADO
-- =====================================================================


-- ---------------------------------------------------------------------
-- PASO 1 · CREAR LAS TRES TABLAS
-- ---------------------------------------------------------------------

-- 1. Tabla Padre
CREATE TABLE persona (
    id_persona INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre VARCHAR(50) NOT NULL,
    apellido VARCHAR(50) NOT NULL,
    correo VARCHAR(80),
    telefono VARCHAR(15)
);

CREATE TABLE cliente_subtipo (
    id_persona INT PRIMARY KEY,
    ciudad VARCHAR(40) NOT NULL,
    puntos INT NOT NULL DEFAULT 0,
    CONSTRAINT fk_cliente_persona
        FOREIGN KEY (id_persona)
        REFERENCES persona(id_persona)
        ON DELETE CASCADE
);
-- 3. Subtipo Empleado
CREATE TABLE empleado (
    id_persona INT PRIMARY KEY,
    cargo VARCHAR(50) NOT NULL,
    sueldo NUMERIC(10,2) NOT NULL,
    fecha_ingreso DATE NOT NULL, 
    CONSTRAINT fk_empleado_persona
        FOREIGN KEY (id_persona)
        REFERENCES persona(id_persona)
        ON DELETE CASCADE
);

-- ---------------------------------------------------------------------
-- PASO 2 · INSERTAR 5 PERSONAS
-- ---------------------------------------------------------------------

INSERT INTO persona (nombre, apellido, correo, telefono)
VALUES
('Juan', 'Perez', 'juan.perez@gmail.com', '987654321'),
('Maria', 'Lopez', 'maria.lopez@gmail.com', '987654322'),
('Pedro', 'Garcia', 'pedro.garcia@gmail.com', '987654323'),
('Lucia', 'Torres', 'lucia.torres@gmail.com', '987654324'),
('Carlos', 'Ramirez', 'carlos.ramirez@gmail.com', '987654325');


-- Consultar los IDs generados
SELECT *
FROM persona
ORDER BY id_persona;


-- ---------------------------------------------------------------------
-- PASO 3 · ASIGNAR LOS SUBTIPOS
-- ---------------------------------------------------------------------

-- Personas 1, 2 y 3 como CLIENTES
INSERT INTO cliente_subtipo (id_persona, ciudad, puntos)
VALUES
(1, 'Pucallpa', 120),
(2, 'Lima', 250),
(3, 'Huanuco', 180);


-- Personas 4 y 5 como EMPLEADOS
INSERT INTO empleado (id_persona, cargo, sueldo, fecha_ingreso)
VALUES
(4, 'Vendedor', 1800.00, '2026-01-15'),
(5, 'Administrador', 2500.00, '2026-02-01');


-- ---------------------------------------------------------------------
-- CHECKPOINT
-- ---------------------------------------------------------------------

SELECT COUNT(*) AS personas
FROM persona;

SELECT COUNT(*) AS clientes
FROM cliente_subtipo;

SELECT COUNT(*) AS empleados
FROM empleado;


-- ---------------------------------------------------------------------
-- PASO 4 · ESPECIALIZACIÓN SOLAPADA
-- ---------------------------------------------------------------------

-- La persona 1, que ya es CLIENTE,
-- también será EMPLEADO.

INSERT INTO empleado (id_persona, cargo, sueldo, fecha_ingreso)
VALUES
(1, 'Atencion al cliente', 1900.00, '2026-03-01');


-- ---------------------------------------------------------------------
-- PASO 5 · CONSULTAR LA JERARQUÍA
-- ---------------------------------------------------------------------

-- J1. Clientes con sus datos completos
SELECT
    p.id_persona,
    INITCAP(p.nombre || ' ' || p.apellido) AS nombre_completo,
    c.ciudad,
    c.puntos
FROM persona p
INNER JOIN cliente_subtipo c
    ON p.id_persona = c.id_persona
ORDER BY p.id_persona;


-- J2. Empleados con sus datos completos
SELECT
    p.id_persona,
    INITCAP(p.nombre || ' ' || p.apellido) AS nombre_completo,
    e.cargo,
    e.sueldo
FROM persona p
INNER JOIN empleado e
    ON p.id_persona = e.id_persona
ORDER BY p.id_persona;


-- J3. ID de la persona que es CLIENTE Y EMPLEADO
SELECT
    p.id_persona,
    INITCAP(p.nombre || ' ' || p.apellido) AS nombre_completo
FROM persona p
INNER JOIN cliente_subtipo c
    ON p.id_persona = c.id_persona
INNER JOIN empleado e
    ON p.id_persona = e.id_persona;


-- ID:
-- 1


-- ---------------------------------------------------------------------
-- PASO 6 · EXPLICACIÓN
-- ---------------------------------------------------------------------

-- ¿Por qué una misma persona pudo aparecer como CLIENTE y EMPLEADO?
--
-- RESPUESTA:
-- Porque los datos comunes de la persona se almacenan una sola vez
-- en la tabla persona. Las tablas cliente_subtipo y empleado solamente
-- guardan los datos propios de cada subtipo y utilizan id_persona como
-- clave primaria y clave foránea. Por eso una misma persona puede
-- pertenecer a ambos subtipos sin repetir sus datos personales.


-- ¿La jerarquía es EXCLUSIVA o SOLAPADA?
--
-- RESPUESTA:
-- La jerarquía es SOLAPADA porque una misma persona puede pertenecer
-- al mismo tiempo a cliente_subtipo y empleado. Se comprobó insertando
-- el mismo id_persona en ambas tablas y realizando consultas que unen
-- persona con cada subtipo.



