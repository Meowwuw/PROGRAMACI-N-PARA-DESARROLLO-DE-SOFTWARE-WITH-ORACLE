...
select fecha from pedido where fecha between '2026-08-01' and '2026-08-31';
...
seletc * from cliente;
select nombre, apellido, ciudad from cliente where ciudad='Pucallpa' or ciudad = 'Lima'
...
--     RESPUESTA:Si usamos únicamente `WHERE categoria NOT IN ('Computo', 'Monitores')`, la 'Alfombrilla Michi XL' NO aparece. 
-- Esto ocurre porque en SQL cualquier comparación contra un valor NULL (su categoría es NULL) da como resultado UNKNOWN,
-- por lo cual la fila es descartada. Para que aparezca, se debe agregar explícitamente `OR categoria IS NULL`.
SELECT * FROM producto WHERE categoria NOT IN ('Computo', 'Monitores') OR categoria IS NULL

-- B10. Clientes cuyo correo es de Gmail.
select * from cliente;
select nombre, correo from cliente where correo like '%@gamil.com%';

-- B11. Clientes cuyo apellido empieza con Q, sin importar mayusculas.
select * from cliente where apellido ilike 'O%';
...
--      Esto es el precio historico del que hablamos: no es un error.
    ...
    SELECT p.nombre AS producto, -- ¡Faltaba la palabra SELECT aquí!
       dp.precio_unit AS precio_historico_cobrado,
...
SELECT prod.id_producto, prod.nombre, prod.categoria, prod.precio
FROM producto prod
