...
select nombre, precio from producto where precio >= 100 and precio <= 800 order by precio asc;
...
select * from cliente where ciudad in ('Lima','Huanuco','Pucallpa');

-- B8. Pedidos que estan PENDIENTE o ENTREGADO.
select id_pedido, estado from pedido where estado in ('PENDIENTE', 'ENTREGADO');
...
--     RESPUESTA:
--     No aparece porque la Alfombrilla tiene su categoria vacia (es NULL). En SQL, los 
--     valores NULL no se pueden comparar con listas o filtros comunes, asi que el sistema 
--     simplemente los ignora y no los muestra.

select * from producto where categoria not in ('Computo', 'Monitores');

-- B10. Clientes cuyo correo es de Gmail.
select nombre, correo from cliente where correo ilike '%@gmail.com';
...
select * from producto where nombre ilike '%Michi%';
...
select nombre, apellido, telefono, ciudad from cliente where telefono is not null;
...
--      RESPUESTA:
--      Porque en SQL el signo '=' no sirve para buscar nulos. Decir 'correo = NULL' no 
--      funciona; para buscar datos vacios o ausentes siempre se debe usar 'IS NULL'.

select * from cliente where correo = null;

-- C16. Accesorios que cuesten menos de 150 soles.
select nombre, stock, precio from producto where categoria = 'Accesorios' and precio < 150 order by precio desc;
...
select nombre, apellido, correo, ciudad 
from cliente 
...
select p.id_pedido, p.fecha, c.apellido, c.ciudad
from pedido p
...
select p.id_pedido, p.fecha, p.estado, c.nombre, c.apellido, c.ciudad
from pedido p
join cliente c on p.id_cliente = c.id_cliente
...
select c.id_cliente, c.nombre, c.apellido, c.ciudad
from cliente c
left join pedido p on c.id_cliente = p.id_cliente
where p.id_pedido is null;

-- D24. Productos que nunca se han vendido.
select pr.id_producto, pr.nombre, pr.categoria, pr.precio
from producto pr
