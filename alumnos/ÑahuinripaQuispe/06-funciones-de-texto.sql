...
select correo,
	lower(correo) as correo_miniscula
from cliente
...
from producto
order by total_caracteres  desc;
...
--- SELECT 'Pedido #' || id_pedido || ' - ' || estado AS resumen
---FROM pedido;
...
select nombre, correo,
	position('@' in correo) as position_arroba
from cliente
where correo is not null;
...
select nombre,
	replace(nombre, 'michi', 'michitore') as nombre_modificado
from producto 
...
SELECT INITCAP(TRIM(nombre)) || ' ' || INITCAP(TRIM(apellido)) AS cliente_limpi
FROM cliente;
...
--      RESPUESTA: 
-- 		No, la funcion TRIM no modifica lo que hay en la tabla.
...
('Carmen', 'Rojas', 'Carme@gmail.com', '945856749'),
('Luz', 'Vazques', 'Luz@gmail.com', '952123985'),
('Susan', 'Ñahuinripa', 'Suy@gmail.com', '937336739'),
('Aracely', 'Quispe', 'Ara@gmail.com', '968999777'),
('Amely', 'Suiza', 'Ame@gmail.com', '955426712');
...
(4, 'Higiene', 1200, '2026-08-29'),
(5, 'Gerente', 3900, '2026-02-14');
...
select p.id_persona,
		...
		c.puntos
from persona p
join cliente_subtipo c on c.id_persona = p.id_persona;
...
VALUES (1, 'Asistente', 3200, '2025-05-26');
...
--
-- RESPUESTA: 
-- Porque las miss no explico que una persona puede ser tanto cliente como empleado.
...
--
-- RESPUESTA:
-- La solapada, lo comprobe mediante la teoria que nos brindo usted miss
-- en la informacion dada dice que la solapada puede ser cliente y empleado
-- asi lo que trabajamos fue la solapada.
